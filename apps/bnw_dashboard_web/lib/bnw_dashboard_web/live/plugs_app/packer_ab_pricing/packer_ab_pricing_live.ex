defmodule BnwDashboardWeb.PlugsApp.PackerAbPricing.PackerAbPricingLive do
  ...
  defp init_args(socket) do
    ...
    args = [
      ...
      %{type: :number, special: :currency, name: :c_fax_6_state,     display_name: "Cattle Fax 6 State", step: 0.01},
      %{type: :number, special: :currency, name: :reg_ab_base_price, display_name: "Reg AB Base Price",  step: 0.01},
      %{type: :number, special: :currency, name: :usda_spread,       display_name: "USDA Spread",        step: 0.01},
      %{type: :number, special: :percent,  name: :plt_yld_percent,   display_name: "PLT YLD%",           step: 0.01},
      %{type: :number, special: :currency, name: :prime,             display_name: "Prime",              step: 0.01},
      %{type: :number, special: :currency, name: :double_r,          display_name: "Double R",           step: 0.01},
      %{type: :number, special: :currency, name: :choice_plus,       display_name: "Choice Plus",        step: 0.01},
      %{type: :number, special: :currency, name: :choice_threshold,  display_name: "Choice Threshold",   step: 0.01},
      %{type: :number, special: :currency, name: :standard,          display_name: "Standard",           step: 0.01},
      %{type: :number, special: :currency, name: :commercial,        display_name: "Commercial",         step: 0.01},
      %{type: :number, special: :currency, name: :no_roll,           display_name: "No Roll",            step: 0.01},
      %{type: :number, special: :currency, name: :low_quality,       display_name: "Low Quality",        step: 0.01},
      %{type: :number, special: :currency, name: :yg1,               display_name: "YG1",                step: 0.01},
      %{type: :number, special: :currency, name: :yg2,               display_name: "YG2",                step: 0.01},
      %{type: :number, special: :currency, name: :yg4,               display_name: "YG4",                step: 0.01},
      %{type: :number, special: :currency, name: :yg5,               display_name: "YG5",                step: 0.01},
      %{type: :number, special: :currency, name: :underweight,       display_name: "Underweight",        step: 0.01},
      %{type: :number, special: :currency, name: :overweight,        display_name: "Overweight",         step: 0.01},
      %{type: :number, special: :currency, name: :plus_30_months,    display_name: "30 + Months",        step: 0.01},
      %{type: :number, special: :currency, name: :plus_30_2_percent, display_name: "30 + Months > 2 %",  step: 0.01},
    ]
  ...
  defp fetch_plugs(socket) do
    ...
    plugs =
      PackerAbPricings.list_plugs(page, per_page, search_col, search)
  ...
  @impl true
  def handle_info({[:packer_ab_pricing, :created_or_updated_add_more], _}, socket) do
    changeset =
  ...
  def handle_event("save", %{"packer_ab_pricing" => plug}, socket) do
    ...
    if changeset.valid? do
      case PackerAbPricings.create_or_update_plug(changeset.data, plug, add_more) do
        {:ok, _plug} ->
          {:noreply, socket}
  ...
  def handle_event("validate", %{"packer_ab_pricing" => plug}, socket) do
  ...
  def handle_event("show_reports", _, socket) do
    {:noreply, assign(socket,
        modal: :show_reports,
        modal_title: "AB Packer Pricing Reports")}
  end
