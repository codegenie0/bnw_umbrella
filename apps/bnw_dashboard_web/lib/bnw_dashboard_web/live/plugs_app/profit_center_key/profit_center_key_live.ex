defmodule BnwDashboardWeb.PlugsApp.ProfitCenterKey.ProfitCenterKeyLive do
  ...
  defp init_args(socket) do
    ...
    args = [
      %{type: :drop_down, special: nil,  name: :company,            display_name: "Company", selected: selected_company, list: companies},
      %{type: :number,    special: :int, name: :profit_center,      display_name: "Profit Center", step: 1},
      %{type: :text,      special: nil,  name: :profit_center_desc, display_name: "Profit Center Description"},
    ]
  ...
  defp fetch_items(plug) do
    company = Map.get(plug, :company, 0)
    |> ProfitCenterKeyCompanies.get_plug()

    Map.put(plug, :company, company)
  end
  ...
  defp fetch_plugs(socket) do
    ...
    plugs =
      ProfitCenterKeys.list_plugs(page, per_page, search_col, search)
  ...
  defp fetch_plug_extra(socket) do
    companies = ProfitCenterKeyCompanies.list_all_plugs()
    assign(socket, plug_companies: companies)
  end
  ...
  def mount(_params, session, socket) do
    ...
    socket =
      ...
      |> assign(page_title: "BNW Dashboard · Plugs Profit Center Key",
                ...
                page: page,
                plug: "profit_center_key",
                selected_search_col: :company,
                search: "",
                per_page: per_page)
      |> fetch_plugs()
      |> fetch_extra()
  ...
  @impl true
  def handle_info({[:profit_center_key_company, :deleted], _}, socket) do
    socket = socket
    |> fetch_extra()
    |> fetch_plug_extra()
    {:noreply, socket}
  ...
  @impl true
  def handle_info({[:profit_center_key, :created_or_updated_add_more], _}, socket) do
    changeset =
  ...
  def handle_event("edit", params, socket) do
    ...
    company = Enum.find(companies, fn x-> x[:key] == cur.data.company end)[:value]
  ...
  @impl true
  def handle_event("edit_companies", _, socket) do
    socket = assign(socket,
      modal: :change_company,
      modal_title: "Manage Companies")
    |> fetch_plug_extra
    {:noreply, socket}
  end
  ...
  def handle_event("save", %{"profit_center_key" => plug}, socket) do
    ...
    if changeset.valid? do
      case ProfitCenterKeys.create_or_update_plug(changeset.data, plug, add_more) do
        {:ok, _plug} ->
          {:noreply, socket}
  ...
  def handle_event("validate", %{"profit_center_key" => plug}, socket) do
  ...
  def handle_event("show_reports", _, socket) do
    {:noreply, assign(socket,
        modal: :show_reports,
        modal_title: "Turnkey Profit Center Key Reports")}
  end
