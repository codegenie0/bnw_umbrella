...
Hooks.infinite_scroll_modal = {
  ...
  scrollAt() {
    return (
      (this.el.scrollTop / (this.el.scrollHeight - this.el.offsetHeight)) * 100
    );
  },
  mounted() {
    this.pending = this.page();
    ...
    this.el.addEventListener("scroll", (e) => {
      if (
        this.pending == this.page() &&
        Math.abs(this.top - this.el.scrollTop) > 50
      ) {
        this.el.scrollTop = this.top;
        this.height = this.el.scrollHeight;
      } else if (
        this.pending == this.page() &&
        Math.abs(this.top - this.el.scrollTop) > 20
      ) {
...
Hooks.infinite_scroll_small_modal = {
  page() {
    return this.el.dataset.page;
  },
  scrollAt() {
  ...
  mounted() {
    ...
    this.el.addEventListener("scroll", (e) => {
      ...
      if (this.pending == this.page() && this.scrollAt() > 90) {
        this.pending = this.page() + 1;
        this.pushEventTo("#sellers-scroller", "load_more", {});             
      }
    });
    UIkit.overflowAuto(this.el, {});
  },
