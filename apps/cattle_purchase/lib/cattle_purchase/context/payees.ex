defmodule CattlePurchase.Payees do
  ...
  def search_query(query) do
    ...
    |> where(
      [p],
      like(p.name, ^query) or
        like(p.vendor_number, ^query) or
        like(p.lienholder, ^query)
    )
    |> order_by([p], fragment("cast(? as unsigned)", p.vendor_number))
    |> Repo.all()
  end
  ...
  defp new_payees() do
    ...
    |> select([tkv, p], %{
      ...
      lienholder:
        fragment(
          "if((right(?, 1) = '&' or right(rtrim(?), 4) = ' and'), ?, null)",
          tkv.name,
          tkv.name,
          tkv.address1
        ),
      address1:
        fragment(
          "if((right(?, 1) = '&' or right(rtrim(?), 4) = ' and'), ?, ?)",
          tkv.name,
          tkv.name,
          tkv.address2,
          tkv.address1
    ...
    |> Enum.map(
      &%{
        id: &1.id,
        name: &1.name,
        vendor_number: "#{&1.vendor_number}",
        lienholder: &1.lienholder
      }
    )
  end
