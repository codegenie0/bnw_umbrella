defmodule PlugsApp.ProjectedBreakevenYards do

  import Ecto.Query
  alias PlugsApp.{
    ProjectedBreakevenYard,
    Repo
  }

  @topic "plugs_app:projected_breakeven_yard"
  ...
  def change_plug(%ProjectedBreakevenYard{} = plug, attrs \\ %{}) do
    ProjectedBreakevenYard.changeset(plug, attrs)
  end

  @doc """
  Validation function used by the modal. Verifies valid date
  """
  def validate(%ProjectedBreakevenYard{} = plug, attrs \\ %{}) do
    plug
    |> change_plug(attrs)
  ...
  """
  def create_or_update_plug(%ProjectedBreakevenYard{} = plug, attrs \\ %{}) do
    plug
    |> ProjectedBreakevenYard.changeset(attrs)
    |> Repo.insert_or_update()
    |> notify_subscribers([:projected_breakeven_yard, :created_or_updated])
  end

  @doc """
  Delete a plug then notify others of its departure
  """
  def delete_plug(%ProjectedBreakevenYard{} = plug) do
    Repo.delete(plug)
    |> notify_subscribers([:projected_breakeven_yard, :deleted])
  end
