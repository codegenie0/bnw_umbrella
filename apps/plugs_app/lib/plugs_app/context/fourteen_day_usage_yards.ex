defmodule PlugsApp.FourteenDayUsageYards do

  import Ecto.Query
  alias PlugsApp.{
    FourteenDayUsageYard,
    Repo
  }

  @topic "plugs_app:fourteen_day_usage_yard"
  ...
  def change_plug(%FourteenDayUsageYard{} = plug, attrs \\ %{}) do
    FourteenDayUsageYard.changeset(plug, attrs)
  end

  @doc """
  Validation function used by the modal. Verifies valid date
  """
  def validate(%FourteenDayUsageYard{} = plug, attrs \\ %{}) do
    plug
    |> change_plug(attrs)
  ...
  """
  def create_or_update_plug(%FourteenDayUsageYard{} = plug, attrs \\ %{}) do
    plug
    |> FourteenDayUsageYard.changeset(attrs)
    |> Repo.insert_or_update()
    |> notify_subscribers([:fourteen_day_usage_yard, :created_or_updated])
  end

  @doc """
  Delete a plug then notify others of its departure
  """
  def delete_plug(%FourteenDayUsageYard{} = plug) do
    Repo.delete(plug)
    |> notify_subscribers([:fourteen_day_usage_yard, :deleted])
  end
