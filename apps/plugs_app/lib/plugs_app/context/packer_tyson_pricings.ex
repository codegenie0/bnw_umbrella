defmodule PlugsApp.PackerTysonPricings do
  ...
  @topic "plugs_app:packer_tyson_pricing"
  ...
  def list_plugs(page \\ 1,
                 ...
                 search_col \\ nil,
                 search \\ "") do
    query = PackerTysonPricing
    |> order_by([plug], [desc: plug.mpc_week_end_date, desc: plug.id])
  ...
  def change_plug(%PackerTysonPricing{} = plug, attrs \\ %{}) do
    PackerTysonPricing.changeset(plug, attrs)
  end

  @doc """
  Validation function used by the modal. Verifies valid date
  """
  def validate(%PackerTysonPricing{} = plug, attrs \\ %{}) do
    plug
    |> change_plug(attrs)
  ...
  """
  def create_or_update_plug(%PackerTysonPricing{} = plug, attrs \\ %{}, add_more \\ false) do
    plug
    |> PackerTysonPricing.changeset(attrs)
    |> Repo.insert_or_update()
    |> notify_subscribers([:packer_tyson_pricing, (if add_more, do: :created_or_updated_add_more, else: :created_or_updated)])
  end

  @doc """
  Delete a plug then notify others of its departure
  """
  def delete_plug(%PackerTysonPricing{} = plug) do
    Repo.delete(plug)
    |> notify_subscribers([:packer_tyson_pricing, :deleted])
  end
