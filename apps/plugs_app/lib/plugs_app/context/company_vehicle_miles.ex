defmodule PlugsApp.CompanyVehicleMiles do
  ...
  def list_plugs(page \\ 1,
                 ...
                 search_col \\ nil,
                 search \\ "") do
    query = CompanyVehicleMile
    |> order_by([plug], [desc: plug.entry_date, desc: plug.id])
  ...
  """
  def create_or_update_plug(%CompanyVehicleMile{} = plug, attrs \\ %{}, add_more \\ false) do
    plug
    |> CompanyVehicleMile.changeset(attrs)
    |> Repo.insert_or_update()
    |> notify_subscribers([:company_vehicle_mile, (if add_more, do: :created_or_updated_add_more, else: :created_or_updated)])
  end
