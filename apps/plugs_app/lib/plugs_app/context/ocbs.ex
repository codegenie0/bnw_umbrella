defmodule PlugsApp.Ocbs do
  ...
  def change_plug(%Ocb{} = plug, attrs \\ %{}) do
    Ocb.changeset(plug, attrs)
  end

  @doc """
  Validation function used by the modal. Verifies valid date
  """
  def validate(%Ocb{} = plug, attrs \\ %{}) do
    plug
    |> change_plug(attrs)
  ...
  """
  def create_or_update_plug(%Ocb{} = plug, attrs \\ %{}, add_more \\ false) do
    plug
      |> Ocb.changeset(attrs)
      |> Repo.insert_or_update()
      |> notify_subscribers([:ocb, (if add_more, do: :created_or_updated_add_more, else: :created_or_updated)])
  end

  @doc """
  Delete a plug then notify others of its departure
  """
  def delete_plug(%Ocb{} = plug) do
    Repo.delete(plug)
    |> notify_subscribers([:ocb, :deleted])
  end
