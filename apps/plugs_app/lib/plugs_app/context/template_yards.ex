defmodule PlugsApp.TemplateYards do
  ...
  def change_plug(%TemplateYard{} = plug, attrs \\ %{}) do
    TemplateYard.changeset(plug, attrs)
  end

  @doc """
  Validation function used by the modal. Verifies valid date
  """
  def validate(%TemplateYard{} = plug, attrs \\ %{}) do
    plug
    |> change_plug(attrs)
  ...
  """
  def create_or_update_plug(%TemplateYard{} = plug, attrs \\ %{}) do
    plug
    |> TemplateYard.changeset(attrs)
    |> Repo.insert_or_update()
    |> notify_subscribers([:template_yard, :created_or_updated])
  end

  @doc """
  Delete a plug then notify others of its departure
  """
  def delete_plug(%TemplateYard{} = plug) do
    Repo.delete(plug)
    |> notify_subscribers([:template_yard, :deleted])
  end
