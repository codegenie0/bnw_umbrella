defmodule PlugsApp.FuelUsageYards do
  ...
  @topic "plugs_app:fuel_usage_yard"
  ...
  def change_plug(%FuelUsageYard{} = plug, attrs \\ %{}) do
    FuelUsageYard.changeset(plug, attrs)
  end

  @doc """
  Validation function used by the modal. Verifies valid date
  """
  def validate(%FuelUsageYard{} = plug, attrs \\ %{}) do
    plug
    |> change_plug(attrs)
  ...
  """
  def create_or_update_plug(%FuelUsageYard{} = plug, attrs \\ %{}) do
    plug
    |> FuelUsageYard.changeset(attrs)
    |> Repo.insert_or_update()
    |> notify_subscribers([:fuel_usage_yard, :created_or_updated])
  end

  @doc """
  Delete a plug then notify others of its departure
  """
  def delete_plug(%FuelUsageYard{} = plug) do
    Repo.delete(plug)
    |> notify_subscribers([:fuel_usage_yard, :deleted])
  end
