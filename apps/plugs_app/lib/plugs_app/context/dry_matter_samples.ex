defmodule PlugsApp.DryMatterSamples do
  ...
  @topic "plugs_app:dry_matter_sample"
  ...
  def list_plugs(yard,
                 ...
                 search_col \\ nil,
                 search \\ "") do
    query = DryMatterSample
    |> where([plug], plug.yard == ^yard)
    |> order_by([plug], [desc: plug.sample_date, desc: plug.id])
  ...
  def change_plug(%DryMatterSample{} = plug, attrs \\ %{}) do
    DryMatterSample.changeset(plug, attrs)
  end

  @doc """
  Validation function used by the modal. Verifies valid date
  """
  def validate(%DryMatterSample{} = plug, attrs \\ %{}) do
    plug
    |> change_plug(attrs)
  ...
  """
  def create_or_update_plug(%DryMatterSample{} = plug, attrs \\ %{}, add_more \\ false) do
    plug
      |> DryMatterSample.changeset(attrs)
      |> Repo.insert_or_update()
      |> notify_subscribers([:dry_matter_sample, (if add_more, do: :created_or_updated_add_more, else: :created_or_updated)])
  end

  @doc """
  Delete a plug then notify others of its departure
  """
  def delete_plug(%DryMatterSample{} = plug) do
    Repo.delete(plug)
    |> notify_subscribers([:dry_matter_sample, :deleted])
  end
