defmodule PlugsApp.Templates do
  ...
  def change_plug(%Template{} = plug, attrs \\ %{}) do
    Template.changeset(plug, attrs)
  end

  @doc """
  Validation function used by the modal. Verifies valid date
  """
  def validate(%Template{} = plug, attrs \\ %{}) do
    plug
    |> change_plug(attrs)
  ...
  """
  def delete_plug(%Template{} = plug) do
    Repo.delete(plug)
    |> notify_subscribers([:template, :deleted])
  end
