defmodule PlugsApp.ProfitCenterKeys do
  ...
  @topic "plugs_app:profit_center_key"
  ...
  def change_plug(%ProfitCenterKey{} = plug, attrs \\ %{}) do
    ProfitCenterKey.changeset(plug, attrs)
  end

  @doc """
  Validation function used by the modal. Verifies valid date
  """
  def validate(%ProfitCenterKey{} = plug, attrs \\ %{}) do
    plug
    |> change_plug(attrs)
  ...
  """
  def create_or_update_plug(%ProfitCenterKey{} = plug, attrs \\ %{}, add_more \\ false) do
    plug
      |> ProfitCenterKey.changeset(attrs)
      |> Repo.insert_or_update()
      |> notify_subscribers([:profit_center_key, (if add_more, do: :created_or_updated_add_more, else: :created_or_updated)])
  end

  @doc """
  Delete a plug then notify others of its departure
  """
  def delete_plug(%ProfitCenterKey{} = plug) do
    Repo.delete(plug)
    |> notify_subscribers([:profit_center_key, :deleted])
  end
