defmodule PlugsApp.OutsideBillings do
  ...
  @topic "plugs_app:outside_billing"
  ...
  def change_plug(%OutsideBilling{} = plug, attrs \\ %{}) do
    OutsideBilling.changeset(plug, attrs)
  end

  @doc """
  Validation function used by the modal. Verifies valid date
  """
  def validate(%OutsideBilling{} = plug, attrs \\ %{}) do
    plug
    |> change_plug(attrs)
  ...
  """
  def create_or_update_plug(%OutsideBilling{} = plug, attrs \\ %{}, add_more \\ false) do
    plug
      |> OutsideBilling.changeset(attrs)
      |> Repo.insert_or_update()
      |> notify_subscribers([:outside_billing, (if add_more, do: :created_or_updated_add_more, else: :created_or_updated)])
  end

  @doc """
  Delete a plug then notify others of its departure
  """
  def delete_plug(%OutsideBilling{} = plug) do
    Repo.delete(plug)
    |> notify_subscribers([:outside_billing, :deleted])
  end
